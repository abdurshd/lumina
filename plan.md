...
- `[ ]` **G2.5** — Persona landing pages (`/for-self-discovery`, `/for-career-pivots`, `/for-coaches`, `/for-schools` — higher-ed only at launch, `/for-hr-teams`) — LOCK: UNLOCKED
...
- `[ ]` **G2.8** — Career-tag programmatic pages `/careers/{slug}` from O*NET — LOCK: UNLOCKED — DEPENDS ON: existing `src/lib/career/`
...
- `[x]` **GX.4** — CSP nonces. Per-request CSP now lives in `src/proxy.ts` (Next 16 renamed `middleware.ts` → `proxy.ts`); production uses `'nonce-XXX' 'strict-dynamic' 'unsafe-eval'` for `script-src`, dev keeps `'unsafe-inline' 'unsafe-eval'` for Turbopack HMR. Static CSP removed from `next.config.ts`. (Style nonces deliberately deferred — `style-src 'self' 'unsafe-inline'` retained because Framer Motion / Tailwind atomics / Radix write inline styles at runtime; high breakage risk for low XSS gain.)
