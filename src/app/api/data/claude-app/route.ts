...
import { parseClaudeExport } from '@/lib/data/claude-app';
...
export async function POST(req: NextRequest) {
  ...
  const consented = await ensureSourceConsent(authResult.uid, 'claude_app');
  if (!consented) {
    return errorResponse(
      'Claude conversations consent not granted. Enable this source in onboarding or settings first.',
      ErrorCode.FORBIDDEN,
      403,
    );
  }
  ...
  try {
    const payload = parseClaudeExport(content);
    const ingestion = buildIngestionResponse('claude_app', payload);
    ...
    try {
      ...
      console.error('[Claude App Agent Analysis]', err instanceof Error ? err.message : err);
    }

    return NextResponse.json({ ...ingestion, agentAnalysis });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to parse Claude export';
