...
import { parseGeminiExport } from '@/lib/data/gemini-app';
...
export async function POST(req: NextRequest) {
  ...
  const consented = await ensureSourceConsent(authResult.uid, 'gemini_app');
  if (!consented) {
    return errorResponse(
      'Gemini conversations consent not granted. Enable this source in onboarding or settings first.',
      ErrorCode.FORBIDDEN,
      403,
    );
  }
  ...
  try {
    const payload = parseGeminiExport(content);
    const ingestion = buildIngestionResponse('gemini_app', payload);
    ...
    try {
      ...
      console.error('[Gemini App Agent Analysis]', err instanceof Error ? err.message : err);
    }

    return NextResponse.json({ ...ingestion, agentAnalysis });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to parse Gemini export';
