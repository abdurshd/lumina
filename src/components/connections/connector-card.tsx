...
export const ConnectorCard = memo(function ConnectorCard({
  ...
  return (
    ...
    >
      <Card className={`transition-colors duration-300 ${isConnected ? 'border-primary/30 bg-primary/[0.03]' : 'hover:border-overlay-strong'}`}>
        ...
        <CardContent>
          {isConnected ? (
            <p className="text-sm text-muted-foreground font-medium">
              ~<AnimatedCounter value={tokenCount ?? 0} /> tokens of data collected
            </p>
          ) : disabled ? (
