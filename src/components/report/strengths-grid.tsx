  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {strengths.map((strength, i) => (
        <Card key={i} className="glass animate-fade-in-up" style={{ animationDelay: `${i * 60}ms` }}>
          <CardContent className="pt-4">
            <div className="flex items-center justify-between mb-1">
              <h4 className="font-medium text-sm font-sans">{strength.name}</h4>
              <span className="text-xs font-medium text-primary font-mono">{strength.score}/100</span>
            </div>
